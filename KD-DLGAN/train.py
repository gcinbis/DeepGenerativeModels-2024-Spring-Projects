...
def setup_training_loop_kwargs(
    ...
    loss       = None, # Loss function: 'da' (default), 'da_kd_agkd', 'da_kd_cgkd', 'da_kd_agkd_cgkd'
):
    ...
    if loss not in ['da', 'da_kd_agkd', 'da_kd_cgkd', 'da_kd_agkd_cgkd']:
        raise UserError('--loss can only be one of the following values: da, da_kd_agkd, da_kd_cgkd, da_kd_agkd_cgkd')
    # -----------------------------------
    ...
    cfg_specs = {
        'low_shot':  dict(ref_gpus=-1, kimg=300,    mb=8,  mbstd=4,  fmaps=1,   lrate=0.002,  gamma=10,   ema=10,  ramp=None, map=2, snap=1),
    ...
    args.loss_kwargs = dnnlib.EasyDict(class_name='training.kd_loss.KDLoss', r1_gamma=spec.gamma)
    if 'agkd' in loss:
